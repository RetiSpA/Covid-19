...
Infected <- data_1$totale_attualmente_positivi[data_1$totale_attualmente_positivi>0]
Morti=data_1$deceduti
Guariti=data_1$dimessi_guariti

data_1$totale_attualmente_positivi[1]
Day=1:(length(Infected))

N <- 4356406  # population of regione
...
t <- 1:100
Piemonte<- data.frame(ode(y = init, times = t, func = SIR, parms = Opt_par))
write_xlsx(Piemonte, path = "Piemonte.xlsx", col_names = TRUE)
