...
Infected <- data_3$totale_attualmente_positivi[data_3$totale_attualmente_positivi>0]
Morti=data_3$deceduti
Guariti=data_3$dimessi_guariti

data_3$totale_attualmente_positivi[1]
Day=1:(length(Infected))

N <- 10060574  # population of regione
...
t <- 1:100
Lombardia<- data.frame(ode(y = init, times = t, func = SIR, parms = Opt_par))
write_xlsx(Lombardia, path = "Lombardia.xlsx", col_names = TRUE)
