...
Infected <- data_11$totale_attualmente_positivi[data_11$totale_attualmente_positivi>0]
Morti=data_11$deceduti
Guariti=data_11$dimessi_guariti

data_11$totale_attualmente_positivi[1]
Day=(max(which(data_11$totale_attualmente_positivi==0))+1):(length(Infected))

N <- 1525271 # population of regione
...
t <- 1:100
Marche<- data.frame(ode(y = init, times = t, func = SIR, parms = Opt_par))
write_xlsx(Marche, path = "Marche.xlsx", col_names = TRUE)
