...
Infected <- data_12$totale_attualmente_positivi[data_12$totale_attualmente_positivi>0]
Morti=data_12$deceduti
Guariti=data_12$dimessi_guariti

data_12$totale_attualmente_positivi[1]
Day=1:(length(Infected))

N <- 5879082  # population of regione
...
t <- 1:100
Lazio<- data.frame(ode(y = init, times = t, func = SIR, parms = Opt_par))
write_xlsx(Lazio, path = "Lazio.xlsx", col_names = TRUE)
