...
Infected <- data_9$totale_attualmente_positivi[data_9$totale_attualmente_positivi>0]
Morti=data_9$deceduti
Guariti=data_9$dimessi_guariti

data_9$totale_attualmente_positivi[1]
Day=(max(which(data_9$totale_attualmente_positivi==0))+1):(length(Infected))

N <- 3729641  # population of regione
...
t <- 1:100
Toscana<- data.frame(ode(y = init, times = t, func = SIR, parms = Opt_par))
write_xlsx(Toscana, path = "Toscana.xlsx", col_names = TRUE)
