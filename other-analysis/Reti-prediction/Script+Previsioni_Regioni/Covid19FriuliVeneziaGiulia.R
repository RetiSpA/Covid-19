...
Infected <- data_6$totale_attualmente_positivi[data_6$totale_attualmente_positivi>0]
Morti=data_6$deceduti
Guariti=data_6$dimessi_guariti

data_6$totale_attualmente_positivi[1]
Day=(max(which(data_6$totale_attualmente_positivi==0))+1):(length(Infected))

N <- 1215220  # population of regione
...
t <- 1:100
FriuliVeneziaGiulia<- data.frame(ode(y = init, times = t, func = SIR, parms = Opt_par))
write_xlsx(FriuliVeneziaGiulia, path = "FriuliVeneziaGiulia.xlsx", col_names = TRUE)
