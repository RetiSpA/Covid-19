...
Infected <- data_8$totale_attualmente_positivi[data_8$totale_attualmente_positivi>0]
Morti=data_8$deceduti
Guariti=data_8$dimessi_guariti

data_8$totale_attualmente_positivi[1]
Day=1:(length(Infected))

N <- 4459477   # population of regione
...
t <- 1:100
EmiliaRomagna<- data.frame(ode(y = init, times = t, func = SIR, parms = Opt_par))
write_xlsx(EmiliaRomagna, path = "EmiliaRomagna.xlsx", col_names = TRUE)
