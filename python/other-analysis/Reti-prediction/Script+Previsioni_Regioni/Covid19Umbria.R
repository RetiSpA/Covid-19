...
Infected <- data_10$totale_attualmente_positivi[data_10$totale_attualmente_positivi>0]
Morti=data_10$deceduti
Guariti=data_10$dimessi_guariti

data_10$totale_attualmente_positivi[1]
Day=(max(which(data_10$totale_attualmente_positivi==0))+1):(length(Infected))

N <- 882015 # population of regione
...
t <- 1:100
Umbria<- data.frame(ode(y = init, times = t, func = SIR, parms = Opt_par))
write_xlsx(Umbria, path = "Umbria.xlsx", col_names = TRUE)
