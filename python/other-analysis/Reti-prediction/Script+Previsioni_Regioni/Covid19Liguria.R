...
Infected <- data_7$totale_attualmente_positivi[data_7$totale_attualmente_positivi>0]
Morti=data_7$deceduti
Guariti=data_7$dimessi_guariti

data_7$totale_attualmente_positivi[1]
Day=(max(which(data_7$totale_attualmente_positivi==0))+1):(length(Infected))

N <- 1550640  # population of regione
...
t <- 1:100
Liguria<- data.frame(ode(y = init, times = t, func = SIR, parms = Opt_par))
write_xlsx(Liguria, path = "Liguria.xlsx", col_names = TRUE)
