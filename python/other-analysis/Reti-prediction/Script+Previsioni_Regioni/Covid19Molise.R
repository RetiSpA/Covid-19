...
Infected <- data_14$totale_attualmente_positivi[data_14$totale_attualmente_positivi>0]
Morti=data_14$deceduti
Guariti=data_14$dimessi_guariti

data_14$totale_attualmente_positivi[1]
Day=(max(which(data_14$totale_attualmente_positivi==0))+1):(length(Infected))

N <- 305617 # population of regione
...
t <- 1:100
Molise<- data.frame(ode(y = init, times = t, func = SIR, parms = Opt_par))
write_xlsx(Molise, path = "Molise.xlsx", col_names = TRUE)
